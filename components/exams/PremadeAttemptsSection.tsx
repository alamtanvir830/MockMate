...
function getFormNumber(examId: string): number {
  const m = examId?.match(/sat-form-(\d+)/)
...
// Serializable shape passed from the server page
export interface ServerAttemptRow {
...
function toDisplay(a: PremadeAttempt): DisplayAttempt {
  return {
    id: a.id,
    examId: a.examId,
...
function serverRowToDisplay(a: ServerAttemptRow): DisplayAttempt {
  return {
    id: a.local_attempt_id,
    examId: `sat-form-${a.form_number}`,
    examTitle: a.exam_title,
    completedAt: a.completed_at,
    rwScaled: a.rw_score,
    mathScaled: a.math_score,
...
export function PremadeAttemptsSection({
  ...
  const [localAttempts, setLocalAttempts] = useState<PremadeAttempt[]>([])
  ...
  useEffect(() => {
    setLocalAttempts(loadAllAttempts())
    setLoaded(true)
  }, [])

  // Merge: localStorage attempts take precedence (richer data); DB fills in the rest
  const attempts = useMemo<DisplayAttempt[]>(() => {
    const localIds = new Set(localAttempts.map(a => a.id))
    const serverOnly = serverAttempts.filter(a => !localIds.has(a.local_attempt_id))
    return [
      ...localAttempts.map(toDisplay),
      ...serverOnly.map(serverRowToDisplay),
    ].sort((a, b) => new Date(b.completedAt).getTime() - new Date(a.completedAt).getTime())
  }, [localAttempts, serverAttempts])
    ...
    <div>
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Results are saved to your account</p>
        </div>
        ...
        {attempts.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {attempts.map((attempt) => {
              const formNum = getFormNumber(attempt.examId)
              const needsPremium = requiresPremiumForResults(formNum) && !satUpgradeUnlocked
              return (
                <div
                  key={attempt.id}
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {needsPremium && (
                        <span className="shrink-0 inline-flex items-center gap-1 rounded-full bg-amber-50 border border-amber-200 px-2 py-0.5 text-[10px] font-semibold text-amber-700">
                  ...
                  {confirmDeleteId === attempt.id ? (
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      {needsPremium ? (
                        ...
                        <Link
                          href={resultsPath(attempt.id, attempt.examId)}
                          className="text-sm font-medium text-emerald-600 hover:text-emerald-500 transition-colors"
                        >
                          View results
                        </Link>
                      )}
                      {attempt.inLocalStorage && (
                        <button
