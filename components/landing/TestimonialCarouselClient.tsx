  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 flex items-center gap-2 pt-3 border-t border-slate-100">
        <div className="h-7 w-7 shrink-0 rounded-full bg-blue-100 flex items-center justify-center text-brand-500 text-[11px] font-semibold select-none">
          {t.initials.charAt(0)}
        </div>
        <div>
          <p className="text-[12px] font-semibold text-slate-800 leading-none">{t.initials}</p>
          {scoreDisplay && (
            <p className="text-[11px] text-brand-500 mt-0.5 font-medium">{scoreDisplay}</p>
          )}
        </div>
      </div>
    </div>
