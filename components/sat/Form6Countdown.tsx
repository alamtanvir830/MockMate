...
// Displays live HH:MM:SS countdowns for Form 6 free access windows.
...
// ── Large banner for the top of /premade/sat ──────────────────────────────────

interface BannerProps {
  expiresAt: string
}

export function Form6CountdownBanner({ expiresAt }: BannerProps) {
  const secs = useCountdown(expiresAt)

  return (
    <div className="mb-6 rounded-xl border border-brand-200 bg-brand-50 px-5 py-4">
      <div className="flex flex-col sm:flex-row sm:items-start gap-3">
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-brand-900 text-sm">
            SAT Form 6 is free for 72 hours
          </p>
          <p className="text-xs text-brand-700 mt-0.5">
            Start Form 6 before the promotional window ends.
          </p>
          <p className="text-xs text-brand-600 mt-1">
            Expires: {formatLocalExpiry(expiresAt)}
          </p>
        </div>
        <div className="shrink-0 flex items-center gap-1.5 rounded-full border border-brand-300 bg-brand-100 px-3 py-1.5">
          <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="h-3.5 w-3.5 text-brand-700 shrink-0" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2m6-2a10 10 0 11-20 0 10 10 0 0120 0z" />
          </svg>
          <time
            aria-live="off"
            className="text-sm font-bold text-brand-800 tabular-nums"
            dateTime={expiresAt}
          >
...
export function Form6DashboardBanner({ expiresAt, actionHref, actionLabel }: DashboardBannerProps) {
  const secs = useCountdown(expiresAt)
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-4 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[12px] text-brand-700 leading-relaxed">
            Take SAT Practice Form 6 free before the timer expires.
          </p>
          <div className="mt-2 flex items-center gap-1.5">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} className="h-3.5 w-3.5 text-brand-600 shrink-0" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2m6-2a10 10 0 11-20 0 10 10 0 0120 0z" />
            </svg>
            <time
              aria-live="off"
...
// ── Compact inline badge for the Form 6 card ─────────────────────────────────
...
export function Form6CountdownBadge({ expiresAt, className }: BadgeProps) {
  const secs = useCountdown(expiresAt)
  ...
  return (
    <span className={className ?? 'inline-flex items-center gap-1 rounded-full border border-brand-200 bg-brand-50 px-2.5 py-0.5 text-[10px] font-semibold text-brand-700 tabular-nums'}>
