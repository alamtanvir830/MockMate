...
function useCountdown(expiresAt: string) {
  // Tick state exists only to trigger re-renders each second.
  // The actual remaining seconds are derived from expiresAt on every render,
  // so the value is always correct even when the prop changes.
...
// Reuses useCountdown and formatters above.
// Used on /dashboard to replace the generic SAT Premium banner for non-premium
// users whose personal 48-hour Form 3 window is currently active.
...
export function Form3DashboardBanner({ expiresAt, actionHref, actionLabel }: DashboardBannerProps) {
  const secs = useCountdown(expiresAt)
