...
// Example: "You have 47 hours and 59 minutes to complete SAT Form 2"
// On expire: "Your free access to SAT Form 2 has expired."
export function SatFreeExamBannerCountdown({ expiresAt }: { expiresAt: string }) {
  ...
  if (parts.expired) {
    return (
      <>
        Your free 48-hour access to SAT Form 2 has expired.{' '}
  ...
  return <>You have {toBannerText(parts)} to complete SAT Form 2</>
}

// Compact badge countdown for card chips.
// Example: "47h 59m left"
// On expire: "expired"
export function SatFreeExamBadgeCountdown({ expiresAt }: { expiresAt: string }) {
  const ms = useCountdown(expiresAt)
  const parts = toParts(ms)
