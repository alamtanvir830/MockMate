...
export const mathQuestionsB4: QBQuestion[] = [
...

  {
    ...
    isWordProblem: true,
    question:
      "A plumber charges a $45 service fee plus $65 per hour for labor. A customer's total bill was $305. Which equation can be used to find h, the number of hours of labor, and how many hours did the plumber work?",
    choices: [
      { label: 'A', text: '45h + 65 = 305; h = 4' },
      { label: 'B', text: '65h + 45 = 305; h = 4' },
      { label: 'C', text: '65h − 45 = 305; h = 5' },
      { label: 'D', text: '45h − 65 = 305; h = 8' },
    ],
    correctAnswer: 'B',
    explanation:
      'The total bill equals the hourly rate times hours worked plus the flat service fee: 65h + 45 = 305. Subtract 45 from both sides: 65h = 260. Divide by 65: h = 260 / 65 = 4. The plumber worked 4 hours.',
    wrongAnswerExplanations: {
      A: 'This reverses which value is the rate and which is the flat fee. The $45 is the flat fee (added once), not the hourly rate, so the coefficient of h must be 65.',
      C: 'Subtracting the service fee rather than adding it — 65h = 305 + 45 = 350, h = 350/65 ≈ 5.4 — produces a non-integer answer, indicating an error in the equation setup.',
      D: 'This uses the flat fee ($45) as the hourly coefficient and subtracts the hourly rate ($65), producing 45h = 370, h ≈ 8.2, which is both algebraically and contextually incorrect.',
    },
    teachingPoint:
      'For "flat fee plus per-unit rate" word problems, set up the equation as: (rate per unit)(units) + flat fee = total. Identify which dollar amount is charged once (flat fee) and which multiplies by the variable (hourly rate) before writing the equation.',
...

  {
    ...
    isWordProblem: true,
    question:
      'A rectangular garden has an area of 40 square meters. The length is 2 more than twice the width. What is the width of the garden, in meters?',
    choices: [
      { label: 'A', text: '4' },
      { label: 'B', text: '5' },
      { label: 'C', text: '8' },
    ...
    correctAnswer: 'A',
    explanation:
      'Let w = width. Then length = 2w + 2. Area equation: w(2w + 2) = 40 → 2w² + 2w = 40 → w² + w − 20 = 0. Factor: find two numbers that multiply to −20 and add to 1: those are 5 and −4. So (w + 5)(w − 4) = 0, giving w = 4 or w = −5. Since width must be positive, w = 4 meters. Check: length = 2(4) + 2 = 10, area = 4 × 10 = 40 ✓.',
    wrongAnswerExplanations: {
      B: 'w = 5 gives length = 12 and area = 60 ≠ 40. This results from solving w² + w = 40 (forgetting to subtract 40 before factoring) or misreading the equation.',
      C: 'w = 8 would be the length when width is 4, not the width itself. Confusing which dimension was defined in terms of the other.',
      D: 'w = 10 gives area = 10 × 22 = 220 ≠ 40. This is not a solution to the equation.',
    },
    teachingPoint:
      'Translate word problems into equations before solving. Here: Area = width × length, and length = 2w + 2, so w(2w + 2) = 40. Rearrange to standard quadratic form, factor, and reject any negative root since dimensions must be positive.',
...

  {
    ...
    isWordProblem: true,
    question:
      'A factory produces defective items at a rate of 3 per every 200 items manufactured. At this rate, how many defective items would be produced in a run of 3,400 items?',
    choices: [
      { label: 'A', text: '17' },
      { label: 'B', text: '34' },
      { label: 'C', text: '51' },
    ...
    correctAnswer: 'C',
    explanation:
      'Set up the proportion: 3/200 = d/3400. Cross-multiply: 200d = 3 × 3400 = 10200. Divide: d = 10200/200 = 51. Alternatively, 3400/200 = 17 batches of 200, and 17 × 3 = 51 defective items.',
    wrongAnswerExplanations: {
      A: '17 is the number of complete batches of 200 items in the run (3400 ÷ 200 = 17), but each batch produces 3 defective items, not 1.',
      B: '34 doubles the number of batches (17 × 2) rather than multiplying by the defect rate of 3 per batch.',
      D: '68 would be 4 defective items per batch of 200, not 3.',
    },
    teachingPoint:
      'Proportion problems require keeping consistent units on each side of the equation: (defective/total) = (defective/total). One useful shortcut: find how many groups of the given batch size fit in the total run, then multiply by the defects per batch.',
...

  {
    ...
    isWordProblem: true,
    question:
      'A store buys a lamp at a wholesale price of $48 and marks it up by 75% to set the retail price. During a clearance sale, the store then discounts the retail price by 30%. What is the final sale price of the lamp?',
    choices: [
      { label: 'A', text: '$50.40' },
      { label: 'B', text: '$57.60' },
      { label: 'C', text: '$58.80' },
    ...
    correctAnswer: 'C',
    explanation:
      'Step 1 — find the retail price: $48 × 1.75 = $84. Step 2 — apply the 30% discount: $84 × (1 − 0.30) = $84 × 0.70 = $58.80.',
    wrongAnswerExplanations: {
      A: '$50.40 = $72 × 0.70, which applies the discount to the wrong base. $72 = $48 × 1.50 (a 50% markup, not 75%).',
      B: '$57.60 = $48 × 1.20, which corresponds to a net 20% increase over wholesale — the arithmetic result of subtracting the discount from the markup (75% − 30% = 45%) and then misapplying it.',
      D: '$67.20 = $84 × 0.80, which uses an 80% multiplier (only a 20% discount) instead of the correct 70% multiplier.',
    },
    teachingPoint:
      'Sequential percentage changes must be applied in order to the current value, not to the original. First multiply by (1 + markup rate) to get retail ($48 × 1.75 = $84), then multiply by (1 − discount rate) to get the sale price ($84 × 0.70 = $58.80). Never subtract the percentages directly.',
...

  {
    ...
    isWordProblem: true,
    question:
      'A box contains tiles labeled with numbers 1 through 20. Tiles labeled with multiples of 3 are red; all other tiles are white. If two tiles are drawn at random without replacement, what is the probability that both tiles are red?',
    choices: [
      { label: 'A', text: '1/19' },
      { label: 'B', text: '3/38' },
      { label: 'C', text: '9/380' },
    ...
    correctAnswer: 'B',
    explanation:
      'First, identify the red tiles (multiples of 3 from 1–20): 3, 6, 9, 12, 15, 18 — that is 6 red tiles out of 20. For drawing 2 red tiles without replacement: P(first red) = 6/20. After removing one red tile, 5 red tiles remain among 19 total: P(second red | first red) = 5/19. P(both red) = (6/20) × (5/19) = 30/380 = 3/38.',
    wrongAnswerExplanations: {
      A: '1/19 comes from computing 5/19 alone — the probability the second tile is red given the first was red, ignoring the probability of the first draw.',
      C: '9/380 results from incorrectly computing (3/20) × (3/19), using 3 red tiles instead of 6.',
      D: '3/20 = 6/40 — this is approximately the probability of drawing one red tile, not two in a row without replacement.',
    },
    teachingPoint:
      'For "without replacement" probability, each draw changes the remaining pool. Multiply the probabilities of each sequential draw: P(A and B) = P(A) × P(B | A). First count the favorable tiles, then apply the multiplication rule for dependent events.',
...

  {
    ...
    isWordProblem: true,
    question:
      'A farmer wants to fence a rectangular pen that has an area of 150 square meters. The length of the pen is 5 meters more than twice its width. What is the total length of fencing needed, in meters?',
    choices: [
      { label: 'A', text: '50' },
      { label: 'B', text: '55' },
      { label: 'C', text: '60' },
    ...
    correctAnswer: 'B',
    explanation:
      'Let w = width. Then length = 2w + 5. Area: w(2w + 5) = 150 → 2w² + 5w − 150 = 0. Using the quadratic formula or factoring: (2w + 20)(w − 7.5) = 2w² − 15w + 20w − 150 — that does not factor cleanly. Try (2w − 15)(w + 10) = 2w² + 20w − 15w − 150 = 2w² + 5w − 150 ✓. So w = 15/2 = 7.5 m or w = −10 (rejected). Width = 7.5 m, length = 2(7.5) + 5 = 20 m. Perimeter = 2(7.5 + 20) = 2(27.5) = 55 m.',
    wrongAnswerExplanations: {
      A: '50 m results from computing perimeter as 2 × width + 2 × (2 × width) = 6w = 6(7.5) = 45 — an error in the length formula — or from misidentifying the dimensions.',
      C: '60 m would correspond to dimensions that do not satisfy the area condition; for example, using w = 10 gives length = 25 and perimeter = 70, not 60.',
      D: '75 m could come from computing the perimeter as width × length (confusing perimeter with area) after correctly finding dimensions.',
    },
    teachingPoint:
      'Fencing problems require finding both dimensions before computing perimeter. Set up the area equation using the algebraic relationship between length and width, solve for the unknown, find both dimensions, then compute perimeter = 2(l + w).',
...

  {
    ...
    isWordProblem: true,
    question:
      'A circular fountain is surrounded by a square patio. The circle fits exactly inside the square, touching all four sides. If the square has a side length of 10 meters, what is the area of the patio not covered by the fountain (the shaded region), in terms of π?',
    choices: [
      { label: 'A', text: '(100 − 25π) m²' },
      { label: 'B', text: '(100 − 10π) m²' },
      { label: 'C', text: '(100 − 100π) m²' },
      { label: 'D', text: '(50 − 25π) m²' },
    ],
    correctAnswer: 'A',
    explanation:
      'Since the circle fits exactly inside the square and touches all four sides, the circle\'s diameter equals the square\'s side length: diameter = 10, so radius r = 5. Area of square = 10² = 100 m². Area of circle = π(5²) = 25π m². Shaded area = 100 − 25π m².',
    wrongAnswerExplanations: {
      B: '(100 − 10π) uses the diameter (10) instead of r² (25) in the circle area formula — forgetting to halve the diameter to get the radius before squaring.',
      C: '(100 − 100π) uses 10² = 100 as if it were r² rather than the diameter; this would make the circle area equal to the square area, leaving zero or negative shaded area, which is impossible.',
      D: '(50 − 25π) halves the square area, perhaps mistaking the square area for a triangle or half-shape.',
    },
    teachingPoint:
      'When a circle is inscribed in a square, the diameter of the circle equals the side of the square. Always find the radius (half the diameter) before computing area = πr². Shaded area = area of outer shape − area of inner shape.',
...

  {
    ...
    isWordProblem: true,
    question:
      'Two roads intersect at a right angle. A park is located 9 kilometers along one road from the intersection, and a library is located 12 kilometers along the other road from the intersection. A straight path is to be built directly from the park to the library. How much shorter is this direct path than the route that goes through the intersection? (Both roads pass through the intersection.)',
    choices: [
      { label: 'A', text: '3 km' },
      { label: 'B', text: '6 km' },
      { label: 'C', text: '9 km' },
    ...
    correctAnswer: 'B',
    explanation:
      'The park and library are at the ends of the two legs of a right triangle with legs 9 km and 12 km. Direct path (hypotenuse): c = √(9² + 12²) = √(81 + 144) = √225 = 15 km. Route through intersection: 9 + 12 = 21 km. Difference: 21 − 15 = 6 km.',
    wrongAnswerExplanations: {
      A: '3 km would be the difference if the direct path were 18 km, which it is not. This may come from mistakenly using 12 − 9 = 3 as the difference between the two legs.',
      C: '9 km is the length of one of the legs, not the difference between the two routes.',
      D: '15 km is the length of the direct path (hypotenuse), not the difference between the two routes.',
    },
    teachingPoint:
      'Recognize that the two roads and the direct path form a right triangle. Use the Pythagorean theorem to find the hypotenuse (direct path), then subtract from the sum of the two legs (route through intersection) to find the savings in distance.',
...

  {
    ...
    isWordProblem: true,
    question:
      'A cylindrical storage tank and a rectangular box have the same volume. The cylinder has a radius of 4 feet and a height of 9 feet. The rectangular box has a length of 12 feet and a width of 6 feet. What is the height of the rectangular box, in terms of π?',
    choices: [
      { label: 'A', text: '(2π/3) feet' },
      { label: 'B', text: 'π feet' },
      { label: 'C', text: '(16π/9) feet' },
      { label: 'D', text: '2π feet' },
    ],
    correctAnswer: 'D',
    explanation:
      'Volume of cylinder = πr²h = π(4²)(9) = π(16)(9) = 144π cubic feet. Volume of rectangular box = length × width × height = 12 × 6 × h = 72h. Set the volumes equal: 72h = 144π → h = 144π ÷ 72 = 2π feet.',
    wrongAnswerExplanations: {
      A: '(2π/3) feet results from computing the cylinder volume as π(4)(9) = 36π (using the radius directly instead of squaring it) and then dividing by 72: 36π/72 = π/2, and then making an arithmetic error.',
      B: 'π feet comes from computing the cylinder volume as π(4)(9) = 36π (forgetting to square the radius) and then dividing correctly by 36: 36π/36 = π — but the base area should be 72, not 36.',
      C: '(16π/9) feet results from dividing 144π by the cylinder height (9) squared (81) rather than by the correct box base area of 72.',
    },
    teachingPoint:
      'When two solids have equal volumes, set their volume formulas equal and solve for the unknown dimension. Volume of cylinder = πr²h (square the radius, not the height). Volume of rectangular box = lwh. Isolate the unknown height by dividing both sides by the box base area (l × w).',
