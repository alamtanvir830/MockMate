import type { MCATSection } from '../types'
import { bbPassages5_01to05 } from './bio-biochem-p01-p05'
import { bbPassages5_06to10 } from './bio-biochem-p06-p10'
import { bbDiscretes5 } from './bio-biochem-discretes'

export const bioBiochemSection5: MCATSection = {
  id: 'bio-biochem',
  title: 'Biological and Biochemical Foundations of Living Systems',
  abbreviation: 'Bio/Biochem',
  questionCount: 59,
  seededCount: 59,
  timeMinutes: 95,
  breakAfterMinutes: 10,
  breakType: 'optional',
  passages: [...bbPassages5_01to05, ...bbPassages5_06to10],
  discreteQuestions: bbDiscretes5,
}
