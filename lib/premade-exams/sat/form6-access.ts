// ── Form 6 Global Free-Access Window ──────────────────────────────────────────
//
// Form 6 uses hardcoded UTC constants for the 72-hour promotional window.
...
export const FORM6_WINDOW_STARTS_AT = '2026-08-12T00:00:00Z'
export const FORM6_WINDOW_ENDS_AT   = '2026-08-15T00:00:00Z'

// ── Types ─────────────────────────────────────────────────────────────────────

export interface Form6FreeWindow {
  startsAt: string
  expiresAt: string
}

export type Form6AttemptStatus = 'none' | 'in-progress' | 'completed' | 'feedback-required'
export type Form6AccessSource  = 'admin' | 'premium' | 'free-window' | 'none'

export interface Form6AccessResult {
  formNumber: 6
  isPremium: boolean
  isAdmin: boolean
  ...
  accessSource: Form6AccessSource
...
/**
 * Returns the Form 6 free window from hardcoded constants.
 ...
 */
export function getForm6FreeWindow(): Form6FreeWindow | null {
  if (!FORM6_WINDOW_STARTS_AT || !FORM6_WINDOW_ENDS_AT) return null
  return {
    startsAt: FORM6_WINDOW_STARTS_AT,
    expiresAt: FORM6_WINDOW_ENDS_AT,
  }
}

// ── Central Form 6 access resolver (pure — no async calls) ───────────────────
//
// Priority order:
...
export function resolveForm6Access(opts: {
  isAdmin: boolean
  isPremium: boolean
  freeWindow: Form6FreeWindow | null
  attemptStatus: Form6AttemptStatus
  attemptId: string | null
  ...
  const base: Omit<Form6AccessResult, 'accessSource' | 'canStart' | 'canResume' | 'canRetake' | 'canCompleteFeedback' | 'canViewResult' | 'lockReason'> = {
    formNumber: 6,
    isPremium,
    isAdmin,
    globalWindowEnabled,
...
/**
 * Returns true if a non-premium user may access Form 6 via the API.
 ...
 */
export async function canNonPremiumAccessForm6Api(userId: string): Promise<boolean> {
  const now = new Date()

  // No constants set → deny
  if (!FORM6_WINDOW_STARTS_AT || !FORM6_WINDOW_ENDS_AT) return false

  // Window hasn't started yet → deny
  if (new Date(FORM6_WINDOW_STARTS_AT) > now) return false

  // Active window → allow
  if (now < new Date(FORM6_WINDOW_ENDS_AT)) return true
