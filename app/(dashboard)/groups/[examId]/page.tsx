...
export default async function GroupDetailPage({
  ...
  // Verify current user has access (must be creator or a recipient)
  ...
  // Build email → submitted_at map by resolving user_ids → emails via admin auth
  const completedByEmail = new Map<string, string>()
  for (const attempt of attempts ?? []) {
    try {
      const { data: authData } = await admin.auth.admin.getUserById(attempt.user_id)
      if (authData?.user?.email) {
        completedByEmail.set(authData.user.email, attempt.submitted_at)
      }
    } catch {
      // skip — non-fatal
    }
  ...
  try {
    ...
    creatorName = creatorAuth?.user?.user_metadata?.full_name ?? creatorEmail.split('@')[0] ?? 'Group creator'
  } catch {
    // skip
  }
  ...
  // Creator
  members.push({
    name: isCreator ? (user!.user_metadata?.full_name ?? user!.email?.split('@')[0] ?? 'You') : creatorName,
    email: isCreator ? user!.email! : creatorEmail,
    role: 'creator',
    isCurrentUser: isCreator,
    status: completedByEmail.has(isCreator ? user!.email! : creatorEmail) ? 'completed' : 'not_started',
    completedAt: completedByEmail.get(isCreator ? user!.email! : creatorEmail) ?? null,
  })
  ...
  for (const r of recipients ?? []) {
    ...
    members.push({
      ...
      status: completedByEmail.has(r.email) ? 'completed' : 'not_started',
      completedAt: completedByEmail.get(r.email) ?? null,
    })
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      {/* Back */}
      <Link href="/groups" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 transition-colors">
