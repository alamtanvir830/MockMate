...
export function EditQuestionsPanel({ examId, questions, editingLocked }: Props) {
  ...
  async function handleRemove(questionId: string) {
    ...
    if ('error' in result) {
      setRemoveError(
        result.error === 'editing_locked'
          ? 'Editing is locked — an attempt has already been submitted for this exam.'
          : result.error,
      )
    } else {
      router.refresh()
        ...
        {editingLocked && (
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
            ...
            <p className="text-xs text-amber-800 leading-relaxed">
              <strong>Editing locked.</strong> One or more participants have already submitted an
              attempt. Modifying questions now would corrupt existing answer records.
            </p>
          </div>
        )}
      </CardHeader>
                    ...
                    <div key={i} className="flex items-center gap-2.5">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          // If this option was the selected correct, track the new value
