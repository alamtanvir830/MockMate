    ...
    <div>
      {showCountdown && freeWindowExpiresAt && (
        <div className="flex items-center justify-center gap-2 bg-amber-50 border-b border-amber-200 px-4 py-2.5">
          ...
          <span className="text-[12px] font-semibold text-amber-700">Your free Form 3 access window is active —</span>
          <Form3CountdownBadge expiresAt={freeWindowExpiresAt} />
        </div>
      )}
      <SATExamTaker form={getSatForm(3, getLatestSatContentVersion(3))} contentVersion={getLatestSatContentVersion(3)} skipPasswordGate isAdmin={isAdmin ?? false} />
    </div>
  )
}
