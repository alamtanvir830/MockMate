...
export function QuestionBankClient() {
  const [recent, setRecent] = useState<QBHistoryEntry[]>([])
  useEffect(() => { setRecent(loadAllQBHistory().slice(0, 3)) }, [])
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Link
          ...
          className="relative rounded-xl border border-brand-200 bg-white p-6 hover:border-brand-400 hover:shadow-sm transition-all group"
        >
          <div className="h-10 w-10 rounded-lg bg-brand-50 flex items-center justify-center mb-4">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} className="h-5 w-5 text-brand-500">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23-.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" />
            </svg>
          </div>
          <div className="flex items-start justify-between mb-1">
            <h2 className="text-lg font-semibold text-slate-900 group-hover:text-brand-700 transition-colors">SAT</h2>
          ...
          <span className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-brand-600 group-hover:text-brand-700">
            Open SAT Premium Question Bank
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-3">
          ...
          {recent.length > 0 && (
            <Link href="/question-bank/history" className="text-sm text-emerald-600 hover:text-emerald-500 font-medium transition-colors">
              View all
            </Link>
          )}
        </div>
        {recent.length === 0 ? (
          <p className="text-sm text-slate-400">No practice sets yet. Start one above.</p>
        ) : (
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100">
            {recent.map(entry => (
              <div key={entry.id} className="flex items-center gap-4 px-4 py-3 hover:bg-slate-50 transition-colors">
                <span className={cn(
                  'shrink-0 inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-bold border',
                  entry.test === 'SAT' ? 'text-blue-700 bg-blue-50 border-blue-200' : 'text-emerald-700 bg-emerald-50 border-emerald-200',
                )}>
                  {entry.test}
                </span>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    {entry.correctCount}/{entry.totalQuestions} correct · {entry.accuracy}% · {new Date(entry.completedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                  </p>
                </div>
                <Link href={entry.reviewUrl} className="shrink-0 text-xs font-semibold text-emerald-600 hover:text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-3 py-1.5 rounded-lg transition-colors">
                  Review
                </Link>
              </div>
            ))}
      ...
      <div className="mt-8 bg-brand-50 border border-brand-100 rounded-xl p-5">
        <div className="flex items-start gap-3">
          <div className="h-8 w-8 rounded-lg bg-brand-100 flex items-center justify-center shrink-0">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} className="h-4 w-4 text-brand-600">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
            </svg>
          </div>
          <div>
            <p className="text-[13px] font-semibold text-brand-900 mb-0.5">Smart Practice Path</p>
