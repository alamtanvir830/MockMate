          ...
          <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col lg:flex-row lg:items-start lg:gap-12">
              ...
              <div className="lg:w-[42%] text-center lg:text-left">
                {/* Eyebrow */}
                <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-brand-500 mb-5">
                  <span className="h-px w-6 bg-brand-500 hidden lg:block" />
                  MockMate
                </p>
                ...
                <h1 className={`${fraunces.className} text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-slate-900 leading-[1.08]`}>
                  ...
                  <span
                    ...
                    style={{ backgroundImage: 'linear-gradient(135deg, var(--color-brand-500), var(--color-brand-600))' }}
                  >
              ...
              <div className="lg:w-[58%] mt-10 lg:mt-0 flex flex-col gap-14">
                ...
                <div className="rounded-2xl border border-blue-100 bg-blue-50 px-6 py-5">
                  <p className="flex items-center gap-2 text-sm font-semibold text-brand-500 mb-3.5">
                  ...
                  <ul className="space-y-2">
                    {satPremiumFeatureLinks.map(({ text, href }) => (
                      <li key={text}>
                        <Link
                          href={href}
                          className="flex items-center gap-2 text-sm text-brand-500 hover:text-brand-600 transition-colors rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
                        >
                          <svg className="h-3.5 w-3.5 shrink-0 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            ...
            <ul className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
              {badges.map(({ icon, label, href }) => (
                <li key={label}>
                  <Link
                    href={href}
                    className="flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
                  >
          ...
          <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {features.map(({ icon, title, description }) => (
                ...
                >
                  <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-brand-500">
                    {icon}
                  </div>
              ...
              <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
                {steps.map(({ number, title, description }) => (
                  <div key={number} className="relative text-center">
                    <div className="relative z-10 mx-auto mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-brand-600 text-sm font-bold text-white">
                      {number}
                    </div>
...
const features = [
  {
    ...
    description:
      'Ten full-length computer-adaptive SAT exams with the same adaptive module routing as test day. Get a 400–1600 estimated score the moment you finish.',
    icon: (
